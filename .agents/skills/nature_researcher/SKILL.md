---
name: Nature Researcher
description: A research assistant that uses Firecrawl to monitor the latest publications in Nature and summarize the most relevant papers based on your background in cold tolerance, photosynthesis, inflorescence development, single-cell in plants, spatial transcriptome in plants, and yield related traits in maize.
---

# Nature Researcher

## Purpose
This skill configures the agent to automatically scrape the recent publications from Nature (nature.com/nature) and filter them based on your specific research expertise in plant genetics, DNA language models, and machine learning.

## Prerequisites
- The **Firecrawl MCP Server** must be running locally and configured in `.mcp.json`.

## Target Journal
- Nature (https://www.nature.com/nature/articles?type=article)
...
1. **Scrape Journal Pages:**
   Try scraping the current issue TOC:
   ```
   firecrawl_scrape(url="https://www.nature.com/nature/articles?type=article", formats=["markdown"])
   ```
   If blocked or insufficient, fall back to `firecrawl_search` with:
   ```
   site:nature.com/articles nature 2026 plant genomics OR language model OR deep learning
   ```
...
```markdown
### 1. [Paper Title](https://www.nature.com/articles/XXXX) - *Nature*
