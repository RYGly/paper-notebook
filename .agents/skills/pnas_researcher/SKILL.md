...
## Purpose
This skill configures the agent to automatically scrape the recent publications from PNAS and filter them based on your specific research expertise in cold tolerance, photosynthesis, inflorescence development, single-cell in plants, spatial transcriptome in plants, and yield related traits in maize.

## Prerequisites
- The **Firecrawl MCP Server** must be running locally (`docker compose up -d`) and configured in the `mcp_config.json`.

## Target Journal
- PNAS (https://www.pnas.org/toc/pnas/current)

## Instructions for the Agent
When the user invokes this skill, follow these exact steps:

1. **Scrape Journal Pages:**
   Use the `firecrawl_scrape` tool (or `firecrawl_search`) to fetch the latest research articles from the PNAS journal.
