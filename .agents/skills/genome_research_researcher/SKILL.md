---
name: Genome Research Researcher
description: A research assistant that uses Firecrawl to monitor the latest publications in Genome Research and summarize the most relevant papers based on your background in plant genomics, DNA language models, and AI.
---

# Genome Research Researcher

## Purpose
This skill configures the agent to automatically scrape the recent publications from Genome Research (genome.cshlp.org) and filter them based on your specific research expertise in plant genetics, DNA language models, and machine learning.
...
## Important: Anti-Bot Considerations
Genome Research is hosted on Cold Spring Harbor Laboratory Press (CSHL Press). If `firecrawl_scrape` on the TOC page is blocked, use `firecrawl_search` with a targeted query such as:
```
"site:genome.cshlp.org 2026" plant genomics OR language model OR deep learning
```
...
2. **Filter & Select:**
   ...
   - **Plant Genomics and Evolution:** crop genomics, regulatory elements, whole-genome duplication, pangenomes.
   - **AI/Deep Learning Methods in Biology:** novel architectures applied to bioinformatics, RNA-seq, sequence modeling, genome annotation.
...
3. **Fetch Abstracts:**
   For each relevant paper, use the `firecrawl_scrape` tool to fetch its dedicated abstract page on genome.cshlp.org to get the full abstract text and DOI.
...
```markdown
### 1. [Paper Title](https://genome.cshlp.org/content/XXXX) - *Genome Research*
...
*   **Link:** [Abstract Page](https://genome.cshlp.org/content/XXXX)
```
